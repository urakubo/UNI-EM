...
```
Where the keys at a highest level (e.g., "2D/3D Filters") corresnpond to the contents of the dropdown menu, and the key "Sub" specifies the number of lower-level contents that are generated below. The key "Func" specifies the functions that are called in UNI_EM\plugins\Plugin.py. Here, check the contents of Plugin.py:
```python
