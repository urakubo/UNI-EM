...
class Sego_dummy:
    ...
    def run(self,input_ids,tile_index_z):
        ...
        original_ids[:in_shape[0],:in_shape[1]] = 0                           ################ For dummy
        original_ids[0,0] = 1                                                 ################ For dummy

        (original_ids_num_pixels_y, original_ids_num_pixels_x) = original_ids.shape
    ...
    def save(self,in_shape):
        ...
        #print('range( 1, self.id_max  + 1 ) ', range( 1, self.id_max  + 1 ))

        for segment_index in range( 1, self.id_max  + 1 ):   ## Changed, only for dummy so far (190129HU)
            print('segment_index ', segment_index)
            print('self.id_counts ', self.id_counts)
